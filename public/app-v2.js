// ---- INICIO DE LA APLICACIÓN Y ESPERA DE DEPENDENCIAS ----

document.addEventListener('DOMContentLoaded', () => {
    // Esta función se asegura de que todas las librerías y datos estén listos antes de ejecutar la app.
    function waitForAppDependencies() {
        const allReady = window.EmojiButton && window.L && window.axios && window.firebase && window.translations && window.countryNamesES;

        if (allReady) {
            // ¡ÉXITO! Todo está cargado. Iniciamos la aplicación.
            initializeApp();
        } else {
            // Si algo falta, esperamos 100ms y volvemos a comprobar.
...
let countryNames; // Se inicializará en initializeApp

// ---- FUNCIÓN DE INICIALIZACIÓN PRINCIPAL ----
function initializeApp() {
    // Asignamos las variables globales que dependen de otros archivos
    countryNames = window.countryNamesES;

    // Configuración de Firebase (ya debería estar listo)
    ...
    window.db = db; // Hacemos la db global para las funciones que la necesiten
    ...
    document.querySelector('button[onclick*="feed"]').onclick = () => showTab('feed');
    document.querySelector('button[onclick*="revelacion"]').onclick = () => showTab('revelacion');
    document.querySelector('button[onclick*="mine"]').onclick = () => showTab('mine');
    document.querySelector('button[onclick*="capsule"]').onclick = () => showTab('capsule');
    document.getElementById('randomThoughtGlobe').onclick = handleRandomThoughtGlobeClick;
    document.getElementById('sendBtn').onclick = handleSendBtnClick;
    document.getElementById('revelacionSendBtn').onclick = handleRevelacionSendBtnClick;
    document.getElementById('capsuleBtn').onclick = handleCapsuleBtnClick;
}
...
function applyTranslations() {
    document.querySelectorAll('[data-translate-key]').forEach(el => {
        ...
        el.innerHTML = window.translations[currentLang][key] || el.innerHTML;
    });
    document.querySelectorAll('[data-translate-key-placeholder]').forEach(el => {
        const key = el.getAttribute('data-translate-key-placeholder');
        el.placeholder = window.translations[currentLang][key] || el.placeholder;
    });
    document.querySelectorAll('[data-translate-key-title]').forEach(el => {
        const key = el.getAttribute('data-translate-key-title');
        el.title = window.translations[currentLang][key] || el.title;
    });
...
function showNotification(messageKey, type = 'info', duration = 3000, context = '') {
    const message = (window.translations[currentLang][messageKey] || messageKey) + context;
...
function mostrarFraseInspiradoraEnTextarea() { 
    const frases = window.translations[currentLang]['inspirational_phrases'];
...
function getQuestionForDate(date) { 
    const questions = window.translations[currentLang]['revelation_questions'];
...
async function handleRevelacionSendBtnClick() { const textarea = document.getElementById('revelacionTextarea'); const text = textarea.value.trim(); if (!text) return; if (window.contienePalabraOfensiva(text)) return; if (!countryReady) { showNotification("js.notification.country_wait", "info"); return; } this.disabled = true; const todayStr = getTodaysDateStr(); try { await window.db.collection('revelations').doc(todayStr).collection('responses').add({ text: text, ts: Date.now(), country: userCountry, user: getAnonUserId(), lang: currentLang }); localStorage.setItem('libre_revelacion_answered', todayStr); clearDraft('revelacionTextarea'); loadRevelacionDiaria(); } catch (e) { console.error("Error saving revelation answer:", e); showNotification("revelation.send_error", "error"); } finally { this.disabled = false; } };
async function showYesterdaysRevelation() {
  ...
  questionEl.textContent = `${window.translations[currentLang]['revelation.yesterdays_question_prefix']} ${yesterdayQuestion}`;

  try {
    const snapshot = await window.db.collection('revelations').doc(yesterdayStr).collection('responses').orderBy('ts', 'asc').get();
    if (snapshot.empty) {
      respuestasEl.innerHTML = `<p class="empty-state" data-translate-key="empty.no_revelation_answers">${window.translations[currentLang]['empty.no_revelation_answers']}</p>`;
      return;
    }
...
function exportAnonId() { const id = getAnonUserId(); window.prompt(window.translations[currentLang]['js.prompt.export_id'], id); }
function importAnonId() { const newId = window.prompt(window.translations[currentLang]['js.prompt.import_id']); if (newId && /^anon-[a-z0-9]{15}$/.test(newId)) { localStorage.setItem("libre_anon_id", newId); loadMyThoughts(); showNotification("js.notification.id_restored", "success"); } else if (newId) { showNotification("js.notification.id_invalid", "error"); } }
// ---- DETECTAR PAÍS ----
...
async function checkNewReplies() { try { const snapshot = await window.db.collection("thoughts").where("user", "==", getAnonUserId()).get(); let hasNewReply = false; for (const doc of snapshot.docs) { const thoughtId = doc.id; const repliesSnap = await window.db.collection("thoughts").doc(thoughtId).collection("replies").orderBy("ts", "desc").limit(1).get(); if (!repliesSnap.empty) { const latestReply = repliesSnap.docs[0].data(); const lastSeen = getLastSeenReplyTimestamp(thoughtId); if (latestReply.ts > lastSeen) { hasNewReply = true; break; } } } const notifEl = document.getElementById("mineTabNotification"); if(notifEl) notifEl.classList.toggle("hidden", !hasNewReply); } catch (e) { console.error("Error checking new replies:", e); } }
async function markRepliesAsSeen(myThoughts) {
    for (const thought of myThoughts) {
        const repliesSnap = await window.db.collection("thoughts").doc(thought.id).collection("replies").orderBy("ts", "desc").limit(1).get();
...
async function handleSendBtnClick() {
  ...
  try {
    this.disabled = true;
    await window.db.collection("thoughts").add({ text: txt, ts: Date.now(), country: userCountry, user: getAnonUserId(), lang: currentLang });
    clearDraft('textarea');
...
// ---- REPORTAR PENSAMIENTOS ----
async function reportThought(thoughtId, buttonElement) {
    if (!confirm(window.translations[currentLang]['js.notification.report_confirm'])) return;
    buttonElement.disabled = true;
    buttonElement.textContent = window.translations[currentLang]['js.notification.reporting'];
    const userId = getAnonUserId();
    const todayStr = new Date().toISOString().split('T')[0];
    const counterDocId = `${userId}_${todayStr}`;
    const counterRef = window.db.collection('report_counters').doc(counterDocId);
    const thoughtRef = window.db.collection('thoughts').doc(thoughtId);
    ...
    try {
        ...
        buttonElement.textContent = window.translations[currentLang]['feed.reported_button'];
    } catch (error) {
        ...
        buttonElement.textContent = window.translations[currentLang]['feed.action_report'];
...
function handleTranslateClick(text) { if (confirm(window.translations[currentLang]['js.notification.translate_confirm'])) { const encodedText = encodeURIComponent(text); const url = `https://translate.google.com/?sl=auto&tl=${currentLang}&text=${encodedText}&op=translate`; window.open(url, '_blank', 'noopener,noreferrer'); } }
async function enviarRespuesta(thoughtId, replyText, respuestasDiv, btn, textarea) { if (!replyText.trim()) return; if (window.contienePalabraOfensiva(replyText)) { showNotification('js.notification.offensive_word', 'error'); return; } btn.disabled = true; textarea.disabled = true; try { const thoughtDoc = await window.db.collection("thoughts").doc(thoughtId).get(); if (!thoughtDoc.exists) { showNotification("js.notification.reply_error_generic", "error"); return; } if (thoughtDoc.data().user === getAnonUserId()) { showNotification("js.notification.reply_error_own", "info"); return; } await window.db.collection("thoughts").doc(thoughtId).collection("replies").add({ text: replyText.trim(), ts: Date.now(), user: getAnonUserId() }); textarea.value = ''; await mostrarRespuestas(thoughtId, respuestasDiv); setTimeout(checkNewReplies, 1000); } catch(e) { console.error("Error saving reply:", e); showNotification('js.notification.reply_error_generic', "error"); } finally { btn.disabled = false; textarea.disabled = false; } }
async function mostrarRespuestas(thoughtId, respuestasDiv) { respuestasDiv.innerHTML = ''; try { const snapshot = await window.db.collection("thoughts").doc(thoughtId).collection("replies").orderBy("ts", "asc").get(); if (snapshot.empty) return; snapshot.forEach(doc => { const data = doc.data(); const replyCard = document.createElement('div'); replyCard.className = 'reply-box'; const textDiv = document.createElement('div'); const span = document.createElement('span'); span.className = 'font-medium'; span.textContent = data.text; textDiv.appendChild(span); const dateDiv = document.createElement('div'); dateDiv.className = 'text-[0.7rem] text-gray-400 mt-1'; dateDiv.textContent = formatearFecha(data.ts); replyCard.append(textDiv, dateDiv); respuestasDiv.appendChild(replyCard); }); } catch(e) { console.error("Error loading replies:", e); respuestasDiv.innerHTML = '<div class="text-[0.7rem] text-red-500">Error al cargar respuestas.</div>'; } }
// ---- ESTRUCTURA DE CARGA DE DATOS ----
async function fetchData() {
    try {
        const now = Date.now();
        const thoughtsPromise = window.db.collection("thoughts").get();
        const capsulesPromise = window.db.collection("capsules").where("openAt", "<=", now).get();
...
function loadGlobalThoughts(page = 1, showSkeleton = false) {
  ...
  if (itemsToDisplay.length === 0) {
      ...
      globalThoughts.innerHTML = `<p class="empty-state">${window.translations[currentLang][messageKey]}</p>`;
      globalPagination.innerHTML = '';
      return;
  }
  ...
  for (let item of pagedItems) {
    ...
    if (data.user !== getAnonUserId()) {
        ...
        if (!isCapsule) { mainActions.innerHTML = `<button class="action-btn" onclick="mostrarCajaRespuesta('${thoughtId}', this)">${window.translations[currentLang]['feed.action_reply']}</button>`; }
        const translateBtn = document.createElement('button');
        translateBtn.className = 'action-btn';
        translateBtn.textContent = window.translations[currentLang]['feed.action_translate'];
        ...
        if (!isCapsule) {
            ...
            reportBtn.textContent = window.translations[currentLang]['feed.action_report'];
        ...
        interactionDiv.innerHTML = `<span class="text-xs text-gray-400">${window.translations[currentLang][isCapsule ? 'feed.capsule_label' : 'feed.own_thought_label']}</span>`;
    }
  ...
  if (globalPagination) { 
      const prev = window.translations[currentLang]['feed.pagination_previous'];
      const next = window.translations[currentLang]['feed.pagination_next'];
      const page_str = window.translations[currentLang]['feed.pagination_page'];
      const of_str = window.translations[currentLang]['feed.pagination_of'];
...
async function loadMyThoughts(page = 1) {
    ...
    try {
        ...
        const myThoughtsSnapshot = await window.db.collection("thoughts").where("user", "==", myAnonId).orderBy("ts", "desc").get();
        ...
        for (const thought of myThoughts) {
            const repliesSnap = await window.db.collection("thoughts").doc(thought.id).collection("replies").get();
            totalRepliesReceived += repliesSnap.size;

            const ecoSnap = await window.db.collection("thoughts").doc(thought.id).collection("eco").get();
            ecoSnap.forEach(doc => countriesReached.add(doc.id));
        }

        personalStatsDiv.innerHTML = `
            <div class="titulo" data-translate-key="mine.stats_title">${window.translations[currentLang]['mine.stats_title']}</div>
            <div class="dato"><span data-translate-key="mine.stats_thoughts_sent">${window.translations[currentLang]['mine.stats_thoughts_sent']}</span> <strong>${myThoughts.length}</strong></div>
            <div class="dato"><span data-translate-key="mine.stats_replies_received">${window.translations[currentLang]['mine.stats_replies_received']}</span> <strong>${totalRepliesReceived}</strong></div>
            <div class="dato"><span data-translate-key="mine.stats_global_reach">${window.translations[currentLang]['mine.stats_global_reach']}</span> <strong>${countriesReached.size}</strong> <span data-translate-key="mine.stats_countries">${window.translations[currentLang]['mine.stats_countries']}</span></div>
        `;
        ...
        if (myThoughts.length === 0) {
            myThoughtsDiv.innerHTML = `<p class="empty-state">${window.translations[currentLang]['empty.my_thoughts']}</p>`;
        } else {
            ...
            for (const thought of pagedItems) {
                ...
                const dateDiv = document.createElement('div');
                dateDiv.className = 'text-[0.7rem] text-gray-400 mt-1';
                dateDiv.textContent = `${window.translations[currentLang]['mine.sent_on']} ${formatearFecha(data.ts)}`;
                const repliesContainer = document.createElement('div');
                repliesContainer.className = 'mt-3 border-t pt-3';
                const lastSeen = getLastSeenReplyTimestamp(thoughtId);
                const repliesSnap = await window.db.collection("thoughts").doc(thoughtId).collection("replies").orderBy("ts", "asc").get();
                ...
                const newBadge = hasNewReplies ? `<span class="ml-2 text-xs font-bold text-red-500">${window.translations[currentLang]['mine.new_replies_badge']}</span>` : '';
                repliesHeader.innerHTML = `${window.translations[currentLang]['mine.replies_received_title']} (${repliesSnap.size}) ${newBadge}`;
                repliesContainer.appendChild(repliesHeader);
                const repliesDiv = document.createElement('div');
                if (repliesSnap.empty) {
                    repliesDiv.innerHTML = `<p class="text-xs text-gray-500">${window.translations[currentLang]['mine.no_replies']}</p>`;
                } else {
...
function loadEstadisticasAnonimas() {
    ...
    el.innerHTML = `<div class="titulo">${window.translations[currentLang]['feed.stats_title']}</div>
                    <div class="dato">${window.translations[currentLang]['feed.stats_today']} <strong>${pensamientosHoy}</strong></div>
                    <div class="dato">${window.translations[currentLang]['feed.stats_week_countries']} <strong>${paisesSemana.size}</strong></div>
                    <div class="dato">${window.translations[currentLang]['feed.stats_record']} <strong>${recordDia}</strong></div>`;
}
function loadMap() {
  ...
  for (let [country, count] of Object.entries(countryCounts)) {
    let coords = window.countryCoords[country] || window.countryCoords["Desconocido"];
    if(coords) {
...
async function handleCapsuleBtnClick() {
    ...
    if (!date || !time || !msg) { showNotification('js.notification.capsule_missing_fields', 'error'); return; }
    ...
    if (openAtDate <= new Date()) { showNotification('js.notification.capsule_in_past', 'error'); return; }
    
    const openAt = openAtDate.getTime();
    try {
        this.disabled = true;
        await window.db.collection("capsules").add({ text: msg, openAt, ts: Date.now(), country: userCountry, user: getAnonUserId(), lang: currentLang });
        ...
        loadMyCapsules();
    } catch (e) { console.error("Error saving capsule:", e); showNotification("js.notification.capsule_error", "error"); } finally { this.disabled = false; }
};
async function loadMyCapsules() { const list = document.getElementById('capsulesList'); if (!list) return; list.innerHTML = ''; try { const now = Date.now(); const snapshot = await window.db.collection("capsules").where("user", "==", getAnonUserId()).orderBy("openAt", "asc").get(); let pendingHtml = ''; let openedHtml = ''; snapshot.forEach(doc => { const data = doc.data(); const createdOn = `${window.translations[currentLang]['capsule.created_on']} ${formatearFecha(data.ts)}`; const scheduledOn = `${window.translations[currentLang]['capsule.scheduled_on']} ${formatearFecha(data.ts)}`; if (data.openAt > now) { const scheduledToOpen = `${window.translations[currentLang]['capsule.scheduled_to_open']} ${formatearFecha(data.openAt)}`; pendingHtml += `<div class="p-4 bg-gray-100 rounded-lg shadow-sm border border-dashed border-gray-300"><div class="flex items-center text-blue-800 font-bold text-sm"><span class="mr-2">🔒</span><span>${scheduledToOpen}</span></div><div class="text-[0.7rem] text-gray-500 mt-1">${createdOn}</div></div>`; } else { const openedOn = `${window.translations[currentLang]['capsule.opened_on']} ${formatearFecha(data.openAt)}`; const card = document.createElement('div'); card.className = 'p-4 bg-white rounded-lg shadow-md'; const header = document.createElement('div'); header.className = 'mb-2 text-blue-700 font-bold text-sm'; header.textContent = openedOn; const p = document.createElement('p'); p.className = 'font-medium text-justify'; p.textContent = data.text; const footer = document.createElement('div'); footer.className = 'text-[0.7rem] text-gray-400 mt-1'; footer.textContent = scheduledOn; card.append(header, p, footer); openedHtml = card.outerHTML + openedHtml; } }); let finalHtml = ''; if (pendingHtml) { finalHtml += `<h3 class="text-lg font-semibold text-blue-900 mb-3">${window.translations[currentLang]['capsule.pending_title']}</h3><div class="space-y-4">${pendingHtml}</div>`; } if (openedHtml) { finalHtml += `<h3 class="text-lg font-semibold text-blue-900 mt-6 mb-3">${window.translations[currentLang]['capsule.opened_title']}</h3><div class="space-y-4">${openedHtml}</div>`; } if (finalHtml) { list.innerHTML = finalHtml; } else { list.innerHTML = `<p class="empty-state">${window.translations[currentLang]['empty.my_capsules']}</p>`; } } catch (e) { console.error("Error loading my capsules:", e); list.innerHTML = '<p class="empty-state text-red-700">Error loading your capsules.</p>'; } }
// ---- NAVEGACIÓN POR TABS ----
...
function createThoughtSkeleton(count = 1) { let html = ''; for (let i = 0; i < count; i++) { html += ` <div class="p-4 bg-white rounded-lg shadow-sm animate-pulse"> <div class="flex items-center space-x-2 mb-3"> <div class="w-12 h-4 bg-gray-200 rounded"></div> </div> <div class="h-4 bg-gray-200 rounded w-5/6 mb-2"></div> <div class="h-4 bg-gray-200 rounded w-4/6 mb-3"></div> <div class="h-3 bg-gray-200 rounded w-1/3"></div> </div> `; } return html; }
function createMyThoughtSkeleton(count = 1) { let html = ''; for (let i = 0; i < count; i++) { html += ` <div class="p-4 bg-white rounded-lg shadow-sm animate-pulse"> <div class="h-4 bg-gray-200 rounded w-5/6 mb-2"></div> <div class="h-4 bg-gray-200 rounded w-4/6 mb-3"></div> <div class="h-3 bg-gray-200 rounded w-1/3 mb-4"></div> <div class="border-t pt-3"> <div class="h-4 bg-gray-300 rounded w-1/2 mb-3"></div> <div class="h-12 bg-gray-200 rounded"></div> </div> </div> `; } return html; }
function createStatsSkeleton() { return ` <div class="stats-box mb-6 animate-pulse"> <div class="h-5 bg-green-200 rounded w-3/4 mb-3"></div> <div class="h-4 bg-green-100 rounded w-1/2 mb-2"></div> <div class="h-4 bg-green-100 rounded w-1/2 mb-2"></div> <div class="h-4 bg-green-100 rounded w-1/2"></div> </div> `; }
function createRevelationSkeleton(count = 1) { let html = ''; for (let i = 0; i < count; i++) { html += ` <div class="revelation-answer animate-pulse"> <div class="h-4 bg-purple-200 rounded w-5/6 mb-2"></div> <div class="h-3 bg-purple-200 rounded w-1/3 mt-2"></div> </div> `; } return html; }
